...
/**
 * DART 공시 뷰어가 보여주는 손익계산서 표를 재현한다.
 *
 * STEP 05는 이 HTML에서 계정명과 금액이 함께 있는 행을 찾아 강조한다
 * (`buildDartOriginalDocument`). 표가 없으면 "원문 표가 보관되어 있지
 * 않습니다" 경고만 남아 조사 결과를 확인할 수 없다.
 */
export function demoDartStatementHtml(input: {
  periodLabel: string;
  currentLabel: string;
  priorLabel: string;
  ...
  const rows = input.accounts
    .map(
      (account) => `      <tr>
        ...
        <td style="text-align:right">${formatAmount(account.amount)}</td>
        <td style="text-align:right">${formatAmount(account.priorAmount)}</td>
      </tr>`,
    )
    .join("\n");
  return `<html><body>
  <p style="text-align:center"><b>연 결 포 괄 손 익 계 산 서</b></p>
  <p style="text-align:center">${input.periodLabel}</p>
  <p style="text-align:right">(단위 : 원)</p>
  <table border="1" cellspacing="0" cellpadding="3">
    <thead>
      <tr>
        <th>과 목</th>
        <th>${input.currentLabel}</th>
        <th>${input.priorLabel}</th>
      </tr>
    </thead>
    <tbody>
${rows}
