...
5. 고정 리포트 유형 `실적 Review`, 선택 기업의 KRX 업종, 선택한 밸류에이션 모델
...
| 선택 가능한 기업 분야 | 수정 | 선택 기업의 KRX 업종을 읽기 전용으로 표시 |
| 밸류에이션 표시 없음 | 추가 | `PER`·`PBR`·`EV/EBITDA`·`DCF` 선택 필드 표시 |
| 선택에 따라 뒤 구성이 달라진다는 안내 | 수정 | 기업 업종의 출처와 밸류에이션 모델의 사용 목적을 짧게 표시 |
...
| `AnalysisContext` | `실적 Review`·KRX 업종·밸류에이션 모델 표시 | 기업 업종, 선택 모델 | 모델 변경 |
...
`reportType`은 서버 고정값 `EARNINGS_REVIEW`다. `companyDomain`은 선택 기업의 KRX 업종을 서버가 스냅샷으로 저장한다. `valuationMethod`는 `PER`, `PBR`, `EV_EBITDA`, `DCF` 중 사용자가 선택하며 기본값은 `PER`다.
...
| `industry` | 동기화된 업종 분류 |
| `listed` | 상장 여부 |
| `mvpEligible` | 현재 서비스에서 선택 가능한 상장사 여부 |
...
3. 요청 body에서 `ownerId`, `userId`, `currentStage`, `reportType`, `companyDomain`을 받지 않는다. `valuationMethod`만 허용된 enum으로 검증한다.
...
|---|---|---|
| 기업 | 선택된 `companyId` 존재 | 기업 존재, 상장, 허용 거래소 |
| 연도 | 값 존재, 정수 | 서버 허용 연도 목록 포함 |
| 분기 | 값 존재 | `Q1`–`Q4` enum |
| 기준일 | `YYYY-MM-DD` 형식 | 실제 달력 날짜 |
| 기업 분야 | 선택 기업 업종 표시 | 선택 기업의 KRX 업종으로 저장 |
| 밸류에이션 | 허용 옵션 선택 | `PER`·`PBR`·`EV_EBITDA`·`DCF` enum |
...
| 서버 인증·세션 | route와 API 소유권 확인 | 필수, `openid-client@6.8.4`와 TD-014·TD-018 적용 |
...
| 일부 표본 기업만 선택 가능 | KRX 상장사 검색·선택 가능 | 필수 |
| 리포트 유형 여러 개 선택 | `실적 Review` 고정 | 필수 |
| 기업 분야 고정 문구 | 선택 기업의 KRX 업종 자동 표시 | 필수 |
| PER 표시 없음 | 복수 모델 선택, 기본값 `PER` | 필수 |
...
- [ ] 비상장 기업은 setup 완료에 사용할 수 없다.
- [ ] 연도, 분기와 기준일이 필수로 검증된다.
- [ ] 리포트 유형은 `실적 Review`, 기업 분야는 선택 기업의 KRX 업종으로 표시·저장된다.
- [ ] 밸류에이션은 네 가지 허용 모델 중 선택할 수 있고 초기값은 `PER`다.
...
1. 기업 마스터의 DART·거래소 동기화 주기와 장애 시 stale 허용 범위
2. `IT 제조업` MVP 적격 업종 분류표와 경계 기업 처리
3. `supportedTargetYears`의 과거·미래 범위
4. 보고서 기준일이 미래 날짜일 때의 허용 여부
5. 대상 연도·분기와 보고서 기준일 사이의 유효 관계

인증 구현은 TD-014·TD-018, 날짜 변환은 TD-015에 따라 확정됐다. 나머지 결정이 남아 있어도 기존 layout 재사용 범위, 기업 선택 필요성, MVP 고정값, 자동 저장, 소유권, 하위 재검증과 다음 단계 이동 계약은 이 명세대로 유지한다.
