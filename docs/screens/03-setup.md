...
| 관련 기술 결정 | TD-011의 PostgreSQL 소유권·작업 상태 원칙 |
...
- `cutoff_date`를 정확한 시각 경계로 바꾸는 timezone·day-end 규칙은 구현 전 별도 기술 결정으로 확정한다.
...
7. 날짜 기준을 정확한 `cutoff_at`으로 바꾸는 timezone·day-end 규칙

이 결정이 남아 있어도 기존 layout 재사용 범위, 기업 선택 필요성, MVP 고정값, 자동 저장, 소유권, 하위 재검증과 다음 단계 이동 계약은 이 명세대로 유지한다.
