...
- STEP 04 질문 card는 질문 본문과 `출처 · 수집 방식`만 표시한다. `확인할 근거`(지표)와 `기간 · 비교`는 질문 문장의 반복이거나 이 화면에서 편집할 수 없는 값이므로 노출하지 않는다. 두 값은 수집·검증 입력으로 그대로 유지하며 근거 범위 확인은 STEP 05에서 한다.
- source dialog는 desktop 2열, mobile 1열이다. icon, title, helper, checkbox를 사용하고 recommendation label은 생략한다.
- 뉴스를 출처로 선택하면 source dialog 안에서 검색 기간의 시작일·종료일을 사용자가 직접 지정한다. 종료일 상한은 보고서 기준일이고 최대 기간은 240일이며, 비워 두면 `대상 분기 시작일 −30일 ~ 보고서 기준일` 기본값을 서버가 적용한다. 확정된 기간은 질문 card의 출처 줄에 함께 표시한다.
